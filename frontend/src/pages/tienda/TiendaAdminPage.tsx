...
export default function TiendaAdminPage() {
  ...
  const isPublished = (p: AdminProduct) => Number(p.es_tienda) === 1 || p.es_tienda === true

  const filteredProducts = products.filter((p) => {
    if (filterTienda === 'published' && !isPublished(p)) return false
    if (filterTienda === 'hidden' && isPublished(p)) return false
    if (selectedCategoryFilter !== 'all' && String(p.categoria_id) !== selectedCategoryFilter) return false
    if (search.trim()) {
  ...
  const handleToggleTienda = async (prod: AdminProduct) => {
    const currentStatus = isPublished(prod)
    const newStatus = !currentStatus
    try {
      await api.post(`/tienda/productos/${prod.id}/toggle`, { es_tienda: newStatus ? 1 : 0 })
      setProducts((prev) =>
        prev.map((p) => (p.id === prod.id ? { ...p, es_tienda: newStatus ? 1 : 0 } : p))
      )
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative w-full sm:w-72">
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary focus:outline-none"
            />
          ...
          <select
            ...
            className="w-full sm:w-48 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-primary cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
          <span className="text-xs font-bold text-slate-500 hidden sm:inline">Estado Tienda:</span>
          <div className="flex bg-slate-100 p-1 rounded-xl text-xs font-bold">
            ...
            >
              Publicados ({products.filter((p) => isPublished(p)).length})
            </button>
            <button
            ...
            >
              Ocultos ({products.filter((p) => !isPublished(p)).length})
            </button>
          </div>
        </div>
      </div>
